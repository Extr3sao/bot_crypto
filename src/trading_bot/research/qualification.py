...
class QualificationResult:
    ...
    def infrastructure_pass(self) -> bool:
        return all(
            c.passed for c in self.checks if c.category == "INFRASTRUCTURE"
        )

    @property
    def methodology_pass(self) -> bool:
        return all(
            c.passed for c in self.checks if c.category == "METHODOLOGY"
        )

    @property
    def economic_pass(self) -> bool:
        return all(
            c.passed for c in self.checks if c.category == "ECONOMIC"
        )

    @property
    def frequency_pass(self) -> bool:
        return all(
            c.passed for c in self.checks if c.category == "FREQUENCY"
        )
    ...
    def evaluate(
        ...
        *,
        ...
        real_data = (
            ...
            and evidence.evidence_class.is_operational
        )
        checks.append(GateCheck(
            ...
            message="" if real_data else "Evidence class insufficient for operational qualification",
        ))
        ...
        ds_ok = (
            dataset is None
            or dataset.evidence_class != EvidenceClass.SYNTHETIC
        )
        checks.append(GateCheck(
            category="METHODOLOGY",
