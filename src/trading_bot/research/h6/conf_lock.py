...
CONFIRMATION_MANIFEST_PATH = Path(
    "docs/external-audit-01/confirmations/CONF-EDGE-002-001.json"
)
...
def assert_confirmation_locked() -> None:
    ...
    if status["source"] == "missing_manifest":
        raise ConfirmationLockViolation(
            f"{CONFIRMATION_ID} manifest missing; cannot verify lock"
        )
    ...
    if now >= locked_until:
        raise ConfirmationLockViolation(
            f"{CONFIRMATION_ID} lock expired at {locked_until}"
        )
