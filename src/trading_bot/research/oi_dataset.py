...
def valid_days_for(
    ledger: list[dict[str, object]], symbol: str, window: tuple[str, str] | None = None
...
def iter_oi_rows(
    ...
    for e in valid_days_for(ledger, symbol, window):
        path = data_dir / str(e.get("normalized_path", "")).replace("data/processed/oi_full_history/", "")
        if not path.exists():
...
def oi_state_at(
    ...
    rows = [
        ...
        if int(r["timestamp_ms"]) <= cutoff_ms  # type: ignore[arg-type]
    ]
...
def completed_hour_snapshots(
    ...
    return [
        ...
        if hour_start_ms <= int(r["timestamp_ms"]) < end  # type: ignore[arg-type]
    ]
...
def hourly_oi_changes(
    ...
    for r in rows:
        by_hour.setdefault(hour_bucket_ms(int(r["timestamp_ms"])) + HOUR_MS, []).append(r)  # type: ignore[arg-type]
        ...
        out.append(
            {
                ...
                "delta_oi": float(cur[-1]["sum_open_interest"]) - float(prev[-1]["sum_open_interest"]),  # type: ignore[arg-type]
                "n_snapshots": len(cur),
            }
        )
    return out
...
def robust_z_oi_change(
    changes: list[dict[str, object]], decision_time_ms: int
) -> dict[str, object]:
    ...
    """
    hist = [
        float(c["delta_oi"])  # type: ignore[arg-type]
        for c in changes
        if int(c["hour_end_ms"]) <= decision_time_ms  # type: ignore[arg-type]
    ]
...
def decision_eligibility_at(
    ...
    cur = [
        r
        for r in iter_oi_rows(ledger, symbol, data_dir)
        if hour_start <= int(r["timestamp_ms"]) < decision_time_ms  # type: ignore[arg-type]
    ]
    cur_ts = sorted({int(r["timestamp_ms"]) for r in cur})  # type: ignore[arg-type]
    ...
    prev = [
        r
        for r in iter_oi_rows(ledger, symbol, data_dir)
        if hour_start - HOUR_MS <= int(r["timestamp_ms"]) < hour_start  # type: ignore[arg-type]
    ]
    checks["previous_hour_reference"] = len({int(r["timestamp_ms"]) for r in prev}) == SNAPSHOTS_PER_HOUR  # type: ignore[arg-type]
