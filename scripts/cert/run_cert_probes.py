...
def static_audits() -> None:
    ...
    imports = sorted(
        {
            ...
            and node.module
            and node.module.startswith("trading_bot")
        }
    )
    forbidden_classes = [
        c
        for c in classes
        if any(k in c.lower() for k in ("riskmanager", "broker", "accounting", "pnlengine"))
    ]
    pnl_sites = [
        n.lineno
        for n in ast.walk(tree)
        if isinstance(n, ast.FunctionDef) and "pnl" in n.name.lower()
    ]
    record(
    ...
    risk_lowering = [
        line.strip()
        for line in RISK_SRC.splitlines()
        if "trades_today" in line and any(op in line for op in ("<", "min(", "lower", "threshold"))
    ]
    record("dp01_24_frequency_kpi_no_runtime_effect", threshold_lowering_lines=risk_lowering)
    demo_kpi = [
        line.strip()
        for line in DEMO_SRC.splitlines()
        if "trades_per_day" in line or "days_ge_3" in line
    ]
    record(
        "dp01_24_kpi_definition",
        kpi_lines=demo_kpi,
        observational_only=not any(
            "check_signal" in line or "check.min" in line for line in demo_kpi
        ),
    )
...
def fixture_probes() -> None:
    ...
    record(
        ...
        no_trade_visible_in_events=any(
            e["event"] == "decision.no_trade" for e in payload["events"]
        ),
    ...
    record(
        ...
        realized_equals_event_sum=abs(
            payload["realized_pnl"] - sum(float(e.get("pnl", 0)) for e in closed_events)
        )
        < 1e-9,
        pnl_source="PaperBroker fills -> broker.check_positions -> risk.record_trade_result",
    )
    # DP01-12/13 single run + trace authority
    run_ids = {e.get("run_id") for e in payload["events"]} | {
        d.get("package", {}).get("run_id") for d in payload["decisions"]
    }
    ...
    chain_ok = (
        any(e["event"] == "decision.verified" for e in payload["events"])
        and any(e["event"] == "paper.position_opened" for e in payload["events"])
        and bool(closed_events)
    )
    ...
    record(
        "dp01_13_decision_trace",
        success_chain_present=chain_ok,
        no_trade_or_reject_chain_present=no_trade_chain,
    )
    ...
    record(
        "dp01_17_run_report_consistency",
        md_keys=sorted(md_values),
    ...
    record(
        ...
        decision_reasons_present=all(
            "reasons" in d or "decision_reasons" in d.get("package", {}) for d in decisions
        ),
        rejected_alternatives_present=any(
            d["package"].get("rejected_alternatives") for d in decisions
        ),
        verifier_verdict_in_decisions=all("verifier" in d for d in decisions),
        debate_fields_present=bool(debate)
        and all(
            k in debate
            for k in ("critiques", "termination_reason", "outcome", "revisions", "counter_evidence")
        ),
...
def adapter_probes() -> None:
    ...
    def _adapt(pkg: Any, props: Any = None) -> Any:
        return adapter.adapt(
            pkg,
            proposals=props if props is not None else proposals,
            evidence_registry=evidence,
            reports=reports,
            now=now,
            prices=prices,
    ...
    no_sizing = ok and not any(
        hasattr(adapted.candidate, f)
        for f in ("notional_usdt", "leverage", "position_size", "stop_loss_pct")
    )
    record(
        "dp02_03_verified_selected_adapts",
        adapted=adapted is not None,
        is_canonical_trade_candidate=ok,
        no_sizing_fields=no_sizing,
        verifier=adapted.verification.verdict.value if adapted else None,
    )

    no_trade = package.model_copy(
        update={"outcome": type(package.outcome).NO_TRADE, "selected_candidate_id": None}
    )
    ...
    tampered_pkg = package.model_copy(
        update={
            "candidate_set": (selected.model_copy(update={"supporting_evidence_refs": ()}),),
            "rejected_alternatives": (),
        }
    )
    unknown_sel = package.model_copy(update={"selected_candidate_id": "proposal:does-not-exist"})
    no_trace_pkg = package.model_copy(
        update={"candidate_set": (selected.model_copy(update={"trace": None}),)}
    )
    record(
    ...
    if pid in proposals:
        ...
        future_props[pid] = proposals[pid].model_copy(
            update={"data_time": now + timedelta(hours=1)}
        )
        record(
            "dp11_future_proposal_verifier",
            adapt=_adapt(package, future_props) is None,
            note="None means fail-closed",
        )
        stale_props = dict(proposals)
        stale_props[pid] = proposals[pid].model_copy(
            update={"data_time": now - timedelta(hours=6), "expires_at": now - timedelta(hours=5)}
        )
        record(
            "dp11_stale_proposal_verifier",
            adapt=_adapt(package, stale_props) is None,
            note="None means fail-closed",
        )
    ...
    record(
        "dp01_06_portfolio_admission",
        admitted=len(portfolio.candidates) == 1,
        portfolio_type=type(portfolio).__name__,
    )

    # DP01-07 risk authority: SELECTED -> REJECT -> zero broker; SELECTED -> ACCEPT -> paper
    settings_risk = demo.build_demo_settings(
        pairs=[("SOL/USDT", True)], mode="paper", kill_switch_enabled=True
    ).risk.model_copy(update={"max_open_positions": 1})
    ...
    signal = Signal(
        symbol="SOL/USDT",
        side="buy",
        strategy_name=adapted.candidate.strategy_id,
        timeframe="5m",
        confidence=0.8,
        price=prices["SOL"],
        stop_loss_pct=None,
    ...
    record(
        "dp07_risk_reject_zero_broker",
        approved=check_reject.approved,
        reason=check_reject.reason,
        blocked_by=check_reject.blocked_by,
        broker_calls_after_reject=0,
    )

    risk_accept = RiskManager(risk=settings_risk, equity=10_000.0)
    check_accept = risk_accept.check_signal(signal)
    sizing_fields = {
        "notional_usdt": check_accept.position_size.notional_usdt
        if check_accept.position_size
        else None
    }
    ...
    approved_signal = Signal(
        ...
        confidence=0.8,
        price=prices["SOL"],
        stop_loss_pct=check_accept.position_size.stop_loss_pct,
...
def dashboard_probes() -> None:
    ...
    record(
        ...
        status_has_visibility_keys=all(
            k in status_payload
            for k in (
                "decisions",
                "funnel",
    ...
    server2.stop()
    with redirect_stdout(io.StringIO()):
        bounded = run_fixture_demo(
            output_dir=ROOT / "reports/cert-probes/after-dash-kill", cycles=2
        )
    record(
        ...
        dashboard_state_reference_only=not hasattr(bounded.state, "set_decision")
        and not hasattr(bounded.state, "approve_risk"),
    )
...
def dynamic_block_probes() -> None:
    ...
    ccxt.binance = _blocked("private_exchange")
    requests.Session.send = _blocked("live")
    ...
    try:
        ...
        ccxt.binance = saved["ccxt_binance"]
        requests.Session.send = saved["requests_send"]
...
def pit_probes() -> None:
    ...
    trace = TraceContext(
        run_id="pit-run", trace_id="pit-trace", correlation_id="pit", causation_id="pit"
    )
    now = datetime(2026, 8, 1, 12, 0, tzinfo=UTC)

    bars = demo_module._bars("SOL", timestamp=int(now.timestamp() * 1000), shape="up")
    ctx = CryptoAssetAgentRegistry().build_context(
        "SOL", bars, bars[-1].timestamp, data_fingerprint="a" * 32, dataset_id="PIT_PROBE"
    )
    ...
    bus = AgentBus(
        ...
        blackboard=__import__("trading_bot.multi_agent", fromlist=["Blackboard"]).Blackboard(
            run_id="pit-run", trace_id="pit-trace"
        ),
        clock=lambda: now,
    )
    ...
    foreign_trace = TraceContext(
        run_id="another-run", trace_id="another-trace", correlation_id="x", causation_id="x"
    )
