...
def _guard_not_canonical_under_pytest(out_dir: Path) -> None:
    ...
    try:
        if resolved == evid or evid in resolved.parents:
            raise RuntimeError(f"TEST_DATASET_WRITE_FORBIDDEN: test attempted to write to V2 evidence dir {evid}")
    except RuntimeError:
        raise
    except Exception:
        pass
...
def process_file_v2(
    ...
    if cls == "VALID":
        ...
        fp_payload = json.dumps(
            {"symbol": symbol, "day": day, "raw_source_sha256": raw_sha, "normalized_sha256": entry["normalized_sha256"], "rows": entry["rows"], "schema_version": SCHEMA_VERSION, "normalizer_version": NORMALIZER_VERSION_V2},
            sort_keys=True,
            separators=(",", ":"),
        )
...
def build_v2_dataset(out_dir: Path = OUT_V2_DEFAULT, raw_dir: Path = RAW) -> tuple[list[dict[str, object]], dict[str, object]]:
    """Build full V2 dataset into out_dir and return (ledger, manifest)."""
    _guard_not_canonical_under_pytest(out_dir) if "PYTEST_CURRENT_TEST" in __import__("os").environ else None
    ...
    for sym in SYMBOLS:
        ...
        per_symbol[sym] = {
            ...
            "invalid_breakdown": {cls: sum(1 for e in invalid if e["classification"] == cls) for cls in sorted({str(e["classification"]) for e in invalid})},
    ...
    hard_fail = any(per_symbol[s]["checksum_failures"] or per_symbol[s]["schema_drift"] for s in SYMBOLS)  # type: ignore[arg-type]
    ...
    manifest = {
        ...
        "excluded_provider_fields": ["count_toptrader_long_short_ratio", "sum_toptrader_long_short_ratio", "count_long_short_ratio", "sum_taker_long_short_vol_ratio"],
        ...
        "totals": {"files": len(ledger), "valid_days": total_valid, "invalid_days": total_invalid, "rows_valid": sum(int(v["rows"]) for v in per_symbol.values()), "valid_days_in_common_window": sum(1 for e in ledger if e["classification"] == "VALID" and str(e["day"]) >= "2021-12-01"), "valid_days_in_common_window_all_symbols": None},  # type: ignore[arg-type]
        "per_symbol": per_symbol,
        "ledger": str((out_dir / "OI_ARCHIVE_DAY_VALIDITY_LEDGER_V2.jsonl").relative_to(REPO)).replace("\\", "/") if out_dir.is_relative_to(REPO) else str(out_dir / "OI_ARCHIVE_DAY_VALIDITY_LEDGER_V2.jsonl"),
        "output_root": str(out_dir.relative_to(REPO)).replace("\\", "/") if out_dir.is_relative_to(REPO) else str(out_dir),
    }
    # compute intersection of valid days across all three symbols in common window
    valid_sets = []
    for sym in SYMBOLS:
        valid_sets.append({str(e["day"]) for e in ledger if e["symbol"] == sym and e["classification"] == "VALID" and str(e["day"]) >= "2021-12-01"})
    if valid_sets:
        manifest["totals"]["valid_days_in_common_window_all_symbols"] = len(set.intersection(*valid_sets))

    fp_src = json.dumps(
        {"schema_version": SCHEMA_VERSION, "normalizer_version": NORMALIZER_VERSION_V2, "files": [{k: e.get(k) for k in ("symbol", "day", "raw_source_sha256", "normalized_sha256", "rows", "dataset_fingerprint", "classification")} for e in sorted(ledger, key=lambda e: (str(e["symbol"]), str(e["day"])))]},
        sort_keys=True,
        separators=(",", ":"),
    )
    manifest["OI_FULL_HISTORY_DATASET_SHA256_V2"] = hashlib.sha256(fp_src.encode("utf-8")).hexdigest()
    # also keep legacy key for compatibility
    manifest["OI_FULL_HISTORY_DATASET_SHA256"] = manifest["OI_FULL_HISTORY_DATASET_SHA256_V2"]
    manifest["ledger_sha256"] = hashlib.sha256((out_dir / "OI_ARCHIVE_DAY_VALIDITY_LEDGER_V2.jsonl").read_bytes()).hexdigest()
...
def main(argv: list[str] | None = None) -> int:
    ...
    if argv and len(argv) >= 1:
        out = Path(argv[0])
    ledger, manifest = build_v2_dataset(out_dir=out)
    return 0
