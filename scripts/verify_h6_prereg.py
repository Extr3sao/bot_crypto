...
def main() -> int:
    ...
    for p in (
        ...
        man_p,
        rec_p,
        DIR / "H6_FEATURE_AUTHORITY_WHITELIST.json",
        DIR / "H6_FAILED_MEMORY_COLLISION_REVIEW.md",
        DIR / "H6_MECHANISM_EVIDENCE.md",
        DIR / "H6_PREREG_CONSISTENCY_AUDIT.json",
        LEDGER,
        DATASET_MANIFEST,
    ):
    ...
    check(
        results,
        "manifest.spec_sha256_matches_spec_bytes",
        man["spec"]["sha256"] == spec_sha,
        spec_sha,
    )
    ...
    check(
        results,
        "commit_record.whitelist_sha_matches",
        rec["whitelist_sha256"] == sha256(DIR / "H6_FEATURE_AUTHORITY_WHITELIST.json"),
    )
    dman_sha = sha256(DATASET_MANIFEST)
    check(
        results,
        "commit_record.oi_dataset_manifest_sha_matches",
        rec["oi_dataset_manifest_sha256"] == dman_sha,
    )
    ledger_sha = sha256(LEDGER)
    check(
        results,
        "commit_record.ledger_sha_matches",
        rec["validity_ledger_evidence_sha256"]
        == sha256(DIR / "OI_DAY_VALIDITY_LEDGER.evidence.jsonl"),
    )
        ...
        json.dumps(
            {
                ...
                "normalizer_version": dm["normalizer_version"],
                "files": files,
            },
            sort_keys=True,
            separators=(",", ":"),
    ...
    check(
        results,
        "manifest_dataset_sha_matches_frozen",
        man["dataset"]["oi_full_history"]["OI_FULL_HISTORY_DATASET_SHA256"]
    ...
    for label, path in (("spec", spec_p), ("manifest", man_p)):
        out = subprocess.run(
            ["git", "diff", commit, "--", str(path)], capture_output=True, text=True
        ).stdout
    ...
    check(
        results,
        "commit_record.tree_is_ancestor_identity",
        subprocess.run(
    ...
    check(
        results,
        "hypothesis_id_consistent",
        spec["hypothesis_id"] == man["hypothesis_id"] == "H6-OI-POSITION-STOCK-01",
    )
    check(
        results,
        "assets_consistent",
        spec["assets"] == man["assets"] == ["BTCUSDT", "ETHUSDT", "SOLUSDT"],
    )
    check(
        results,
        "window_consistent",
        spec["common_window_utc"]["start"] == man["common_window_utc"][0] == "2021-12-01T00:00:00Z"
        and spec["common_window_utc"]["end"]
        == man["common_window_utc"][1]
        == "2026-09-10T23:59:59Z",
    )
    check(
        results,
        "decision_timeframe_1h",
        spec["decision_timeframe"]["bucket"] == "1h" and man["decision_timeframe"] == "1h",
    )
    check(results, "horizon_1h", spec["decision_timeframe"]["primary_holding_horizon_hours"] == 1)
    check(
        results,
        "entry_next_hour_open",
        spec["entry_timing"].startswith("next 1h bar OPEN")
        and man["entry_timing"] == "NEXT_HOUR_OPEN",
    )
    check(
        results,
        "exit_same_hour_close",
        spec["exit_timing"].startswith("same next 1h bar CLOSE")
        and man["exit_timing"] == "NEXT_HOUR_CLOSE",
    )
    check(
        results, "stop_none", spec["stop_invalidation"].startswith("NONE") and man["stop"] == "NONE"
    )
    check(
        results, "cooldown_none", spec["cooldown"].startswith("NONE") and man["cooldown"] == "NONE"
    )
    check(
        results,
        "expansion_only",
        spec["threshold_rule"]["expansion_condition"] == "z_oi >= +1.0"
        and spec["threshold_rule"]["contraction_condition"].startswith("NOT USED"),
    )
    check(
        results,
        "robust_z_mad_definition",
    ...
    check(
        results,
        "cost_single_canonical_10bps",
        spec["cost_model"]["BASE_TOTAL_ROUND_TRIP_COST_BPS"] == 10
        and man["cost_model"]["BASE_TOTAL_ROUND_TRIP_COST_BPS"] == 10
        and spec["cost_model"]["COST_SENSITIVITY_BPS"] == [0, 10, 20, 40],
    )
    check(
        results,
        "funding_excluded_with_limitation",
    ...
    check(
        results,
        "h5_comparator_honest",
        spec["orthogonality_gates"]["H5_PNL_CORRELATION"].startswith(
            "NOT_EVALUABLE_FROM_PERSISTED_EVIDENCE"
        ),
    )
    check(
        results,
        "minimum_N_frozen",
        spec["minimum_N"]["per_asset_trades"] == 30
        and spec["minimum_N"]["pooled_trades"] == 100
        and man["gates"]["minimum_N"] == {"per_asset": 30, "pooled": 100},
    )
    check(
        results,
        "statistical_gates_frozen",
    ...
    check(
        results,
        "causal_eligibility_frozen",
        "DECISION_ELIGIBILITY_AT_T" in av
        and "data_time <= T" in av["DECISION_ELIGIBILITY_AT_T"]
        and "NOT directly control" in av["ARCHIVE_DAY_VALIDITY"],
    )
    check(
        results, "twelve_snapshot_rule_in_spec", "12 distinct 5m OI snapshots" in json.dumps(spec)
    )
    check(
        results,
        "future_gap_invariant_in_spec",
        "gap strictly AFTER T must not alter eligibility" in json.dumps(spec),
    )
    ...
    check(
        results,
        "whitelist_contains_oi_fields",
        {"sum_open_interest", "sum_open_interest_value"} <= allowed,
    )
    check(
        results,
        "whitelist_excludes_ratio_fields",
        not (allowed & forbidden) and len(forbidden) == 4,
    )
    check(
        results,
        "spec_forbidden_matches_whitelist",
        set(spec["data_authority"]["forbidden_fields"]) == forbidden,
    )

    # ---- consistency audit ----
    check(
        results,
        "consistency_audit_pass",
        audit["commit_allowed"] is True and audit["UNRESOLVED_FIELDS"] == [],
    )
    ...
    check(
        results,
        "no_execution_counters",
        _zero(spec["H6_EXECUTIONS"])
        and _zero(spec["H6_BACKTESTS"])
        and spec["PERFORMANCE_OBSERVED"] is False
        and man["H6_EXECUTIONS"] == 0
        and man["H6_BACKTESTS"] == 0
        and man["PERFORMANCE_OBSERVED"] is False,
    )
    forbidden_keys = (
        "result",
        "returns",
    ...
    check(
        ...
        and man["status_at_prereg"] == "PREREGISTERED_NOT_EXECUTED",
    )
    check(
        results,
        "no_sweep_language",
        "NO parameter sweep" in json.dumps(spec) and "NO_PARAMETER_SWEEP" in json.dumps(man),
    )
    ...
    check(
        ...
        "H6_FAILED_MEMORY_COLLISION_REVIEW = PASS" in review or "`PASS`" in review,
    )
    mech = (DIR / "H6_MECHANISM_EVIDENCE.md").read_text(encoding="utf-8")
    check(
        results,
        "mechanism_evidence_tiers",
        "OFFICIAL_EXCHANGE_DOC" in mech and "No claim of profitability" in mech,
    )
    ...
    (DIR / "H6_SELF_VERIFICATION.json").write_text(
        json.dumps(out, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    print(
        f"H6_SELF_VERIFICATION={verdict} ({len(results) - len(failed)}/{len(results)} checks passed)"
    )
    for r in failed:
