...
## ADR-MA-0009 - Natural multi-proposal agreement evidence authority (MA-4, DEF-MA4-003)

- **Status**: Decidido y verificado en `CP-MA4-POSTCERT-001` (CERT-MA4-002 sobre `15e7048`).
- **Defecto observado**: con acuerdo direccional natural (Momentum LONG + Trend LONG, mismo run/trace), el motor unia refs de soporte derivadas del debate en `DecisionCandidate.supporting_evidence_refs`; el verificador (binding exacto por ADR-MA-0007) rechazaba el paquete naturalmente valido. Observado en POC01 (fixture y bounded real-market). Fail-closed correcto, rechazo semanticamente incorrecto.
- **Decision (MODEL A)**: la autoridad de `DecisionCandidate.supporting_evidence_refs` es el conjunto de evidencia del `TradeProposal` terminal (identidad canonica por conjunto, ADR-MA-0007). El motor ya no une evidencia de acuerdo/debate en ese campo: la corroboracion permanece en `DebateReport` y en los campos de procedencia del candidato (`debate_id`, `debate_outcome`, `material_dissent`, `challenged_by`). El verificador NO cambia (v3 se mantiene; no hubo cambio semantico del verificador, el builder se conforma al binding ya autoritativo). Rechazado MODEL B (closure ampliado re-derivable): reabriria superficie de inyeccion sin necesidad, dado que la procedencia del debate ya es observable.
- **Protecciones preservadas**: foreign-run/foreign-trace/unknown/future evidence, stripping parcial/total, swap, duplicados, promocion de counter-evidence, falsificacion UNRESOLVED/INSUFFICIENT — todos REJECT (validator 21/21; matriz CP-MA4-003).
- **Evidencia**: `tests/unit/multi_agent/test_cp_ma4_003_agreement_evidence.py` (escenarios reales MA2→MA3→MA4→verificador, sin paquetes fabricados), `scripts/validate_ma4_decision_engine.py` 21/21 (`natural_multi_proposal_agreement_verified`, `unauthorized_agreement_evidence_rejected`), CERT-MA4-002 (`docs/ma4/CERT_MA4_002.md`), compat demo (`docs/ma4/CERT_DEMO_PAPER_01_COMPAT_MA4_002.md`).
