...
## ADR-MA-0005 — Evidence-based Decision Engine V1 (MA-4)

- **Estado**: Decidido y verificado en `CP-MA-004` (implementación; certificación pendiente en CERT-MA4-001).
- **Decisión**: la selección de oportunidades es un motor determinista sin ejecución que transforma `TradeProposals + MetaRanker V1 + DebateReports + AgentEvidence` en un `DecisionPackage` inmutable con resultados cerrados `SELECTED/REJECTED/NO_TRADE` (`NO_TRADE` de primera clase, nunca error). Sin votación mayoritaria: tres agentes correlacionados no son tres evidencias independientes; la decisión razona sobre evidencia única, outcome de debate y disenso material.
- **Resolución de linaje (MA-4B)**: `TerminalProposalResolver` reduce cada linaje de revisión a un proposal terminal por raíz; ciclos y ramas marcan la raíz inválida (fail-closed, nunca seleccionable). El original permanece auditable en `revision_lineage`.
- **Elegibilidad (MA-4C)**: gates deterministas en orden fijo — linaje inválido, futuro, stale (data_time > 24h), expirado, trace inválido, evidencia no registrada/futura, evidencia vacía, outcome de debate `INSUFFICIENT_EVIDENCE`, `UNRESOLVED` y conflicto direccional sin debate — todos fail-closed. Ninguna regla de scoring resucita un candidato inelegible; un score bruto superior no salta gates.
- **Procedencia de score (MA-4D/E)**: `DecisionScoreBreakdown` declara fuente por componente (`META_RANKER` para el score canónico V1 sobre el proposal terminal; `DEBATE` para `counter_evidence_materiality`, materialidad media de challenges sin responder). Anti-doble-conteo: las revisiones ya están reflejadas en la confianza del proposal revisado (dentro del score MetaRanker), así que solo challenges *standing* alimentan el componente DEBATE. Sin `MIN_DECISION_SCORE` — se difiere a MA-5/L6 (MA-4I).
- **Selección (MA-4J/G)**: orden canónico (ids ordenados), score DESC con tie-break por identidad de proposal; todo no-seleccionado se preserva como `RejectedAlternative` con reasons tipadas (`LOWER_RANKED_ALTERNATIVE` para elegibles, motivo de gate para inelegibles). El disenso material sobrevive en `material_dissent` + `challenged_by` (MA-4F).
- **Verificación independiente (MA-4P)**: `DecisionPackageVerifier` determinista separado del builder (separación forzada vía `VerificationMetadata`): consistencia de outcome, existencia/eligibilidad del seleccionado, sin selección stale/expirada, alternativas rechazadas completas, reasons válidas, descomposición de score reproducible y sin doble conteo, evidencia material resoluble, sin conflicto bloqueante en el seleccionado.
- **Autoridad temporal**: `decision_time` se inyecta desde el run clock (`AgentBus.now()` aguas arriba); cero llamadas wall-clock en `decision.py`/`contracts/decision.py` (AST-audited). Sin campos de ejecución (tamaño, leverage, broker, credenciales) — RISK_CALLS=0, BROKER_CALLS=0, LIVE_CALLS=0.
...
## ADR-MA-0007 - Selected evidence authority binding, verifier v2 (MA-4)

- **Status**: Decidido y verificado en `CP-MA-004.2` (reparacion de CERT-MA4-001-RETRY-001, hallazgo de CERT-MA4-001-RETRY sobre `85848fc`).
- **Defecto observado**: `DecisionPackageVerifier` comprobaba la resolvibilidad iterando los refs del propio paquete; una lista vacia (tamper EVIDENCE_STRIPPED sobre el candidato seleccionado) pasaba vacuamente y el paquete falsificado resultaba VERIFIED, violando el invariante de elegibilidad MA-4 (evidencia vacia -> INELIGIBLE).
- **Decision (modelo de autoridad)**: la autoridad de evidencia del candidato seleccionado se origina en el `TradeProposal` terminal canonico. El verificador reconstruye de forma independiente `selected_candidate.final_proposal_id -> terminal proposal` y exige `supporting_evidence_refs != vacio`, `terminal.evidence_refs != vacio` y igualdad canonica por conjunto de identidades (el orden no tiene semantica). La lista del paquete nunca es su propia fuente de verdad. Version del verificador: `decision-package-verifier-v2`.
- **Rechazos cubiertos**: EVIDENCE_STRIPPED, PARTIALLY_STRIPPED, SWAPPED (aunque el sustituto este registrado, en el run actual y con trace consistente), ADDED, DUPLICATED, UNKNOWN, FOREIGN_RUN, FOREIGN_TRACE. Ademas `final_proposal_binding`: re-resolucion independiente del linaje; apuntar a un ancestro no terminal se rechaza. `run_authority_reports_registry`: todo reporte entregado al verificador debe pertenecer al run del paquete, incluso si ningun candidato lo referencia.
...
## ADR-0024 — Execution reliability primitives + external audit reconciliation (EXTERNAL-AUDIT-RECONCILIATION-01)

- **Estado**: Decidido y verificado en checkpoint EXTERNAL-AUDIT-RECONCILIATION-01 sobre HEAD `a282fcc`.
- **Contexto**: una auditoría externa citó identidades (`Extr3sao/Bot-Trading`, `AETHEL/AstraQuant`) que no resuelven públicamente y commits (`3c274cc`, `bb46c92`, `39578a6`) que SÍ existen en el object DB de este repo (side branches, no ancestros de HEAD). Verdict de identidad: **SAME_HISTORY_DIFFERENT_NAME** — los hallazgos se re-auditan contra HEAD, nunca se aplican como hechos (docs/external-audit-01/IDENTITY_RECONCILIATION.md).
- **Gap matrix**: 2 PRESENT_AND_SUFFICIENT (orphan detection, startup reconciliation — paper runtime), 6 PRESENT_BUT_INCOMPLETE (journal, fill idempotency, feed dead-man, stat validation, portfolio, adapter conformance), 4 ABSENT (TradeIntent identity, ambiguous ACK, cancel confirmation, StrategyHealth), 1 NOT_RUNTIME_REACHABLE (CORRELATION_CLUSTER enum declarado sin implementación). DEF-CORR-001 (correlación estática 0.85/0.55) **REJECTED**: grep verificado en HEAD y en los commits auditados — no existen constantes de correlación; solo un constante de layout charting y un label `correlation_group`.
- **Decisión**: implementar solo los gaps ABSENT de ejecución como módulos nuevos `src/trading_bot/execution/{intent,journal,feed_guard}.py` con primitivas deterministas: `TradeIntent` (intent_id = SHA-256 canónico de campos económicos; metadata/wall-clock excluidos de identidad), `IdempotentSubmitGate` (ECONOMIC_ORDERS_PER_INTENT <= 1), `AmbiguousAckRecovery` (timeout != safe retry; ADOPT/CONTROLLED_RETRY/BLOCK), `FillLedger` (fill idempotency por venue_fill_id), `ExecutionJournal` (11 estados canónicos append-only con validación FSM), `FeedDeadManGuard` (stale feed bloquea entradas nuevas). No se re-implementa lo PRESENT_AND_SUFFICIENT; no se toca POC01 (frozen); LIVE permanece DISABLED; no se importa vn.py.
- **RFCs registrados** (diseño, sin implementación): RFC-STRATEGY-HEALTH-01, RFC-CORRELATION-REGIME-01 (causal invariant: future data -> past state unchanged), RFC-EXCHANGE-CONFORMANCE-01; gaps stat (Sharpe bootstrap CI, P(Sharpe>0), permutation) mapeados a StrategyAdmissionVerifier como próximo checkpoint.
- **Evidencia**: `tests/unit/execution/` 53 tests (incl. matriz adversarial completa con invariante ECONOMIC_ORDERS_PER_INTENT <= 1 bajo retry 2x/10x, ACK perdido/tardío/duplicado, fills duplicados/out-of-order, cancel timeout/reject/late, orphan, restart con open order/position, feed stale/reconnect); Ruff clean; mypy strict clean (7 files execution/); full unit 766 passed / 1 failed (baseline conocido binance-vs-bybit por `.env` local `EXCHANGE_ID=bybit`, no relacionado); dependency-closure guard PASSA tras trackear los módulos nuevos. POC01_RUNTIME_CHANGED=0, PNL_CHANGED=0, TRADE_COUNT_CHANGED=0, LIVE_CALLS=0, FALSE_SUCCESS=0.

## ADR-0025 — Intelligence & execution hardening (INTELLIGENCE-AND-EXECUTION-HARDENING-01)

- **Estado**: Decidido y verificado sobre base `814f0e4`; POC01 congelado e intacto (RUNTIME/PNL/TRADE_CHANGED=0, LIVE_CALLS=0, FALSE_SUCCESS=0).
- **Track A (execution runtime)**: los adapters runtime-reachables son Binance/Bybit (CCXT) y Bitunix spot/futures; OKX y Kraken NO existen (whitelist los excluye por diseño). Ninguno tenía los primitivos integrados (identidad, journal, ACK recovery, fill ledger, cancel confirmation). Se implementa `execution/gateway.py::ExecutionGateway` como puente de autoridad único (intent -> intent_id -> cloid -> journal -> VenuePort -> recovery/fills/reconcile/execution_ready) con short-circuit post-submit y retries controlados gobernados por el submit gate (ECONOMIC_ORDERS_PER_INTENT <= 1 bajo cualquier ambigüedad). `execution/conformance.py` añade ExchangeAdapterConformanceSuite V1 (identity, metadata, account, positions, orders, fills, submit, query, cancel, timeout ambiguity, reconnect, startup reconciliation, stale-feed). Status: EXECUTION_RELIABILITY_FOUNDATION_ONLY — lista exacta de integración faltante en runtime_execution_authority_map.md.
- **Track B (strategy health)**: `strategies/health.py` con celdas condicionales (strategy x version x asset x timeframe x regime x window), 7 estados (incl. LEGACY_PAPER_BASELINE e INSUFFICIENT_EVIDENCE), hysteresis de ventanas consecutivas, transiciones SOLO como propuestas (B3: no self-modification; recovery QUARANTINED exige research artifact y aterriza en MONITORING; RETIRED terminal). Verificador independiente (BUILDER != VERIFIER) valida sample adequacy, dominios métricos, window identity, PIT, baseline/regime binding, hysteresis y legalidad de transición.
- **Track C (regime v2)**: el RegimeEngine existente (PIT, descriptivo) se EXTIENDE por composición; `research/regime_v2.py` deriva el estado canónico de 6 dimensiones (trend_direction, trend_strength, volatility, market_structure, stress, correlation_regime) + RegimeTransition para atribución + classify_regime_cell (ELIGIBLE/NOT_ELIGIBLE/SHADOW_ONLY/INSUFFICIENT_SAMPLE). Sin activar filtrado en POC01.
- **Track D (dynamic correlation)**: `portfolio/correlation.py` — retornos PIT alineados (sin forward-fill de retornos cero), rolling correlation causal (ventanas trailing), edges por umbral, smoothing EMA causal, hysteresis NORMAL/FUSED. Invariante de causalidad probado: mutar TODAS las observaciones futuras deja los estados pasados byte-idénticos. Solo evidencia de contexto/riesgo, nunca alpha.
