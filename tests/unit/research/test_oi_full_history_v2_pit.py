...
def test_pit_future_same_day_gap_does_not_change_T(tmp_path: Path) -> None:
    ...
    days = [f"2024-06-{d:02d}" for d in range(1, 16)]  # 15 days to satisfy 336 rolling changes horizon? Use lighter: need 720 changes for robust; eligibility still false for rolling but current-hour check is the core PIT part; we verify byte-identical feature input at T via causal state
    ...
    assert before_elig["checks"]["current_hour_complete"] is True, "baseline must have eligible completed hour (non-vacuous)"
    ...
    assert after_elig["checks"]["current_hour_complete"] == before_elig["checks"]["current_hour_complete"]
...
def test_pit_past_gap_before_T_may_change_eligibility(tmp_path: Path) -> None:
    ...
    shard = out / "BTCUSDT" / "BTCUSDT-oi-5m-2024-06-15.jsonl"
    lines = [ln for ln in shard.read_text().splitlines() if json.loads(ln)["timestamp_ms"] != cur_hour_observation]
...
def test_pit_exact_duplicate_collapse_authorized(tmp_path: Path) -> None:
    ...
    zpath.with_name(zpath.name+".CHECKSUM").write_text(hashlib.sha256(zpath.read_bytes()).hexdigest()+f"  {zpath.name}\n")
...
def test_test_isolation_guard_forbids_canonical_write(tmp_path: Path) -> None:
    ...
    try:
        ...
        try:
            ...
            assert False, "should have raised TEST_DATASET_WRITE_FORBIDDEN"
...
def test_oi_sign_rule_blocks_contraction_even_when_z_would_pass() -> None:
    from trading_bot.research.h6.feature_engine import H6FeatureEngine, CompletedHourPrice, CompletedHourOI, compute_signal
    from trading_bot.research.h6.contracts import SignalDirection, PriceDirection
    ...
    decision_time = datetime(2024,6,5,12,0,tzinfo=timezone.utc)
    ...
    prev = CompletedHourOI(hour_close_time=decision_time - timedelta(hours=1), oi_last_snapshot=100.0, snapshot_count=12)  # delta=-8
    # Need 336 history entries to pass eligibility; craft median=-10 MAD=1
    # Create 336 deltas where median=-10, MAD=1
    hist = [(decision_time - timedelta(hours=i+2), -10 + (1 if i%2==0 else -1)) for i in range(336)]
    ...
    prev2 = CompletedHourOI(hour_close_time=decision_time - timedelta(hours=1), oi_last_snapshot=100.0, snapshot_count=12)  # delta 0.1 small
    ...
    prev3 = CompletedHourOI(hour_close_time=decision_time - timedelta(hours=1), oi_last_snapshot=100.0, snapshot_count=12)  # delta 10
    ...
    hist3 = [(decision_time - timedelta(hours=i+2), float((i%5)-2)) for i in range(336)]  # values -2..2 median 0 MAD ~1
    feature3 = engine.compute_feature_state("BTCUSDT", decision_time, price, cur3, prev3, hist3)
    ...
    feature3_down = engine.compute_feature_state("BTCUSDT", decision_time, price_down, cur3, prev3, hist3)
...
def test_whitelist_forbids_ratio_fields_and_non_whitelisted() -> None:
    from trading_bot.research.h6.whitelist import assert_field_allowed, H6ForbiddenFeatureAccess
    for f in ["count_toptrader_long_short_ratio","sum_toptrader_long_short_ratio","count_long_short_ratio","sum_taker_long_short_vol_ratio"]:
...
def test_canonical_v2_dataset_sha_still_matches() -> None:
    ...
    m = json.loads((REPO / "docs/external-audit-01/oi-full-history-02/OI_FULL_HISTORY_DATASET_MANIFEST_V2.json").read_text())
    assert m["OI_FULL_HISTORY_DATASET_SHA256_V2"] == "16779b7d2eff0dc9e56015c444c2dbe7b67ef083e6cf1877a024a6de74098d99"
