...
def test_projection_shape_is_frontend_ready() -> None:
    row = project_health_row(_identity(), _snapshot(), StrategyHealthState.MONITORING, last_transition="admitted", reason="clean")
    payload = row.to_dict()
...
def test_next_gate_is_informational_per_state() -> None:
    ...
    insufficient = project_health_row(_identity(), _snapshot(), StrategyHealthState.INSUFFICIENT_EVIDENCE)
...
def test_table_projection_is_deterministic_and_sorted() -> None:
    ...
    b = (_identity(asset="BTC"), _snapshot(), StrategyHealthState.DEGRADED, "degraded", "2 bad windows")
    c = (_identity(strategy_id="alpha", asset="ETH"), _snapshot(), StrategyHealthState.QUARANTINED, "quarantined", "governance")
...
def test_projection_has_no_trading_controls() -> None:
    ...
    forbidden = {"disable", "enable", "promote", "quarantine", "retire", "execute", "submit", "order"}
