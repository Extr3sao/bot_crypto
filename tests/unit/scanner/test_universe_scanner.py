...
def test_iteration_completed_emitted_on_kill_switch() -> None:
    ...
    # `field_name` es un string loop var (nombres de keys del structlog event
    # payload) que shadow el ``field`` de dataclasses importado en el header.
    # Renombramos a `field_name` para evitar noise de noqa y mejorar grep.
    for field_name in (
        "duration_ms",
        "pairs_processed",
        "pairs_active",
