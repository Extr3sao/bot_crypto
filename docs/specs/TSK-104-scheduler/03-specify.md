...
### 6.3 Invariante de `connector_reinjector` (R1 opcion b)

> Contrato pineado por **R1 opcion b** (detallada en `04-plan.md` §R1):
> `OHLCVScheduler.connector_reinjector(new_mode)` levanta
> `ConfigurationError` cuando:
>
> 1. `self._connector_factory is None` (scheduler construido sin
>    connector; valido solo para `research`/`backtest`), Y
> 2. `new_mode in {paper, live, shadow_live}` (modos que requieren
>    connector real CCXT).
>
> En ese caso, elevar `ConfigurationError` con el siguiente mensaje:
>
> ```
> Cannot switch to mode {new_mode!r}: no connector_factory was injected.
> Either rebuild the scheduler with a ConnectorFactory, or stay in
> research/backtest. See tasks/decisions.md (ADR-0014).
> ```
>
> Esto resuelve el mode-flip edge case: el scheduler se construye
> con `mode=backtest` (sin factory, valido) y luego el caller intenta
> flip a `live` — el flip falla FAST con error claro, NO crashea
> mid-iteration. Cubierto por test
> `tests/unit/scheduler/test_connector_reinjector.py::test_connector_reinjector_raises_when_factory_none_and_mode_needs_connector`.
>
> Pine contract: este metodo es el UNICO punto de cambio de
> connector en runtime. Cualquier intento de mutar
> `self._connector` directamente esta vetado por code review.

### 6.4 Concurrencia scheduler (writer) vs scanner (reader) — R7

> Contrato pineado por **R7** (detallada en `04-plan.md` §R7):
>
> **Writer** = `OHLCVScheduler._process_one_pair` → `OHLCVFetcher.fetch_and_cache`
> → `OHLCVStore.upsert_ohlcv` (TSK-102).
>
> - El writer usa `executemany` atomico (single SQL statement).
> - La conexion ya tiene `PRAGMA journal_mode=WAL` +
>   `isolation_level=None` + `PRAGMA foreign_keys=ON` (TSK-102
>   setup; pine in `tests/unit/storage/test_ohlcv_store.py`).
>
> **Reader** = `UniverseScanner` (TSK-103) → `OHLCVStore.get_ohlcv`.
>
> - El reader invoca `.fetchall()` DESPUES de que el writer
>   retorne de `.executemany`. NUNCA concurrente con un upsert
>   parcial.
>
> **Asyncio scheduling invariant**: tanto `scheduler.run_once()`
> como `scanner.run()` son coroutines que se intercalan en
> `await` points. NUNCA en operaciones SQL sincronas
> (`sqlite3.Connection.execute`/`executemany`/`fetchall` son
> sincronos por diseno en CPython; el scheduler cede el control
> solo cuando hace `await source.fetch_one(...)` o `await
> asyncio.sleep(...)`).
>
> Cubierto por code-review al boundary del writer (`fetch_and_cache`
> retorna la lectura canonica del store DESPUES del upsert; ver
> `src/trading_bot/market_data/ohlcv_fetcher.py`). Si una
> regression futura revela partial-write leaks, anadir
> `TSK-104.3b.6` (test pine en `test_concurrent_read_while_writer_active.py`)
> — pine contract es backward-compat: el reader ve siempre
> snapshots consistentes porque el writer termina su upsert antes
> de retornar al scheduler, y el scheduler cede el control solo
> via `await`, no durante `executemany`.
